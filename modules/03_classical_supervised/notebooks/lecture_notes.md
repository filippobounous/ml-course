...
Work the problem set in `../problems/README.md`. Implement IRLS logistic regression in `../problems/starter.py` (reference in `../problems/_reference/solutions.py`). Build the portfolio artifact in `../../../portfolio/03_tabular_benchmark/`: logistic / random forest / XGBoost / LightGBM on Covertype with full ROC/PR and calibration reporting.
